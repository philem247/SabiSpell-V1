...
path = os.path.join(res_dir, "mipmap-xxxhdpi", "ic_launcher_foreground.webp")

if os.path.exists(path):
    with Image.open(path) as img:
        print(f"ic_launcher_foreground.webp: format={img.format}, size={img.size}, mode={img.mode}")
        # Count transparent/non-transparent pixels
        datas = img.getdata()
        transparent = 0
        non_transparent = 0
